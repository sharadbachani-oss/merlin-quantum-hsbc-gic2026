...
**Prior work already receipted.** Tuned temporal baseline AUC 0.9118 / AUPRC 0.5532 (IEEE-CIS); gray zone [0.630, 0.819] frozen, 2,362 transactions, 26% fraud, in-band AUC 0.614. Hardware: 128-qubit feature-extraction card on IBM Heron (206 circuits × 8,192 shots, 2,720 two-qubit gates per circuit, replica agreement 0.0039, `daaucejvpcac73dd232g`); 128-qubit training-free generative card (524,288 samples, `dab6therrl7c7386flo0`); ULB generative augmentation on QCi Dirac-3 (+1.3% AUPRC over an Ising-pair sampler). These establish the instrument, the controls and the discipline the PoC inherits.
...
| Field-carrying response feature (amounts as fields, time gaps as couplings) | **built; floor passed** (4/9 features CV R² < 0.8); label test at 14 qubits: +0.001 AUPRC vs raw-input control, CI [−0.005, +0.007] — null at simulable scale | raw neighbourhood inputs, relational, shuffled, random | exact oracle; IonQ via Braket in PoC | 2.9 h CPU; ~4,000 circuits × 1,000 shots planned | PoC bar: same paired lift at 40–60 qubits, CI > 0 |
