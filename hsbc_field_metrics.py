...
def label(nproc=4):
    ...
    for i in idx_all:
        ...
        if js:
            ...
            raw[i] = r
    t0 = time.time(); done = 0
    cached = os.path.join(HERE, "field_resp_features.npy")
    if os.path.exists(cached) and not os.environ.get("HSBC_TIMES"):
        resp[idx_all] = np.load(cached); print("loaded cached response features (no oracle re-run)", flush=True)
    else:
    ...
    for name, fn in arms.items():
        ...
        for f in folds:
            ...
            k = max(1, int(round(0.10 * len(f)))); thr = np.sort(p)[-k]; yhat = (p >= thr).astype(int)   # fixed alert budget: top 10% of the fold
            f1s.append(float(f1_score(y[f], yhat))); prs.append(float(precision_score(y[f], yhat, zero_division=0))); rcs.append(float(recall_score(y[f], yhat)))
        out[name] = dict(auprc_mean=float(np.mean(aps)), auprc_sd=float(np.std(aps)), auc_mean=float(np.mean(aucs)), f1_mean=float(np.mean(f1s)), precision_mean=float(np.mean(prs)), recall_mean=float(np.mean(rcs)), alert_budget='top 10% of each fold', folds=aps)
    ...
    res = dict(quantum_execution_note='response oracle simulated exactly on CPU (14 qubits); stratified arm-train subsample 8,000 with fraud ratio preserved + 2,362-transaction gray zone = 10,362 neighbourhoods', gray_zone=dict(lo=float(lo), hi=float(hi), n=int(len(gz_idx)), fraud_rate=float(y[gz_idx].mean())), K=K, tau_s=TAU, alpha=ALPHA, g=G, times=TIMES,
               arms=out, paired_lift_response_vs_raw=paired, oracle_wall_s=round(time.time() - t0, 1))
    json.dump(res, open(os.path.join(HERE, "hsbc_field_response_metrics.json"), "w"), indent=1); print("paired lift vs +raw_neighbourhood:", paired); print("done")
